...
energy_filtered <-  energy %>% 
  filter(year == "2019", country != "Antarctica", country != "Western Sahara") %>% 
  group_by(country,variable) %>% 
    ...
    mutate(perc_hydro=round(elec_hydro/elecprod, 3),
          ...
          perc_renew_total=(perc_hydro+perc_solar+perc_wind+perc_renew_other)) 


energy_new <- energy_filtered %>% 
  filter(perc_renew_total != 0) %>% 
  slice_min(perc_renew_total, n=20)
...
Since the correlation value for Co2 emission and usage renawable energy is negative, we can say that as usage of renawable energy goes up, we can reduce CO2 emission by upto 31%.\
