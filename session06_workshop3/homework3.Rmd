...
You will first start with analyzing the `weight` of participants in kilograms. Using visualization and summary statistics, describe the distribution of weights. How many observations are we missing weights from? Use also the skim function.
...
The minimum value of weight is 30, the maximum value is 181, the median is 64, the 1/4 quantile is 56, the 3/4 quantile is 76, and the total number of missing values is 1004. The distribution of weights looks lightly skewed to the right. 
...
kable(ci_yrbss,
      caption="CI 3 or more times active")
...
The 95% confidence interval is [0.323,0.338] calculated with both, formula and bootstrap.
...
Although almost similiar, there is a positive relationship between being physically active for at least 3 days a week and weight. The result is somehow counter-intuitive, since we thought that more exercise in children would reduce there weight. This might be explained by the reason that people who stay highly active physically are part of physically demanding sports like basket, football and similiar and have thus a higher percentage of muscle or are just bigger.
...
p+geom_errorbarh(aes(y=forcats::fct_inorder(director),xmin=lower,xmax=upper),
  ...
  geom_text(aes(label = mean, x = mean, y = director), vjust = -2, size=7, color = "black") +
  geom_text(aes(label = lower, x = lower, y = director), vjust = -3, size=5, color = "black") +
  geom_text(aes(label = upper, x = upper, y = director), vjust = -3, size=5, color = "black") +
